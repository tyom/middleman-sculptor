  ...
  .directive('iframe', function() {
    return {
      ...
      link: function($scope, $element, $attrs) {
        ...
        $element.on('load', updateIframeHeight);

        angular.element(window).on('resize', _.debounce(updateIframeHeight, 200));

        $scope.$watch('navIsHidden', function() {
          // Ideally should get the delay from the `.glypto-nav` transition length
